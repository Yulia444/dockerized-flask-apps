...
def profiles(id):
    ...
    try:
        ...
        for key, value in study_goals["goals"].items():
            for goal in teachers[id-1].goals.split(' '):
                if goal == key:
                    id_goals.append(value)
    teacher_schedule = json.loads(teachers[id-1].schedule)
    return render_template("profile.html", teacher = teachers[id-1], goal_values = id_goals,
